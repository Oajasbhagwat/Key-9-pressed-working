                      ...
                      sFONT* Font, COLOR Color_Background, COLOR Color_Foreground )
{
  ...
  // show an integer right-aligned -----------


//-------------------------------------------------------------------------
//  Show an integer, right aligned ((instead of the usual left aligned )
//------------------------------------------------------------------- 
  void ShowNum2(POINT x1,POINT y1,int32_t N1,sFONT* pFont, COLOR bkc,COLOR penc )
{

     //int32_t N2 ;
 int16_t Nb=0, i;        //Sb1=0,Sb2=0 ;
 int8_t  N_Arr[20]={0},w1 ;  // S_Arr[20]={0}, i;
   
   while (N1>0) { N_Arr[Nb] = N1 % 10;  Nb++; N1 /=10 ;}  // at the end, N_ARR[0,1,2] = 3,9,7 & Nb=3(3 digits) ,if N1 was 793 
             //Sb2 = Nb;   while (Nb>0)  { S_Arr[Sb1] = N_Arr[Nb-1] ; Sb1++; Nb--; }  // at the end, S_Arr[0,1,2]= 7,9,3 & Sb2=3 (no. of digits
         w1= pFont->Width ; x1-=w1 ;  // x1 now points to top left corner 
    
                 // colr_bitmap(x1+ (i*27), y1,  pMap+S_Arr[i]*120,  26,30, colr);      // large font
       for (i=0;i<Nb; i++) { if ( (i==3)||(i==5) )  {GUI_DisChar(x1 -i*w1 ,y1, ',', pFont, bkc,penc); x1-=w1; }
                  if(N_Arr[i]==0)GUI_DisChar(x1 -i*w1 ,y1, '0', pFont, bkc,penc);
                      else GUI_DisNum (x1- i*w1,y1, N_Arr[i], pFont, bkc,penc);  // font  ,for initial testing 
...
void GUI_Show(void)
{
  ...
  if (sLCD_DIS.LCD_Dis_Column > sLCD_DIS.LCD_Dis_Page) { //Horizontal screen display
    ...
    GUI_DrawLine(0, 310, LCD_WIDTH, 310, GREEN, LINE_SOLID, DOT_PIXEL_2X2);
   
    GUI_DrawLine(0, 10, LCD_WIDTH, 300,     GREEN, LINE_SOLID, DOT_PIXEL_2X2);      // diagomal
    GUI_DrawLine(0, 300, LCD_WIDTH, 10,     GREEN, LINE_SOLID, DOT_PIXEL_2X2);      // diagoma2
